...
const Main = () => {
    ...
    function drawmap(response){
        ...
        for(let i=0;i<count_spot;i++) {
            ...
            for (let j = 0; j < count_coordinate; j++) {
                if(hiking_arry[j].PMNTN_SN===spot_array[i].PMNTN_SN) //코스 코드가 같다면 좌표 저장
                {  spot_path2.push(hiking_arry[j]);
                linePath_co.push(new kakao.maps.LatLng(hiking_arry[j].paths_x,hiking_arry[j].paths_y));}
            }
        ...
        for(let j=0; j<count_spot;j++) {
            ...
            positions.push({
                content:'<div style="width:100%; padding:5px;">' +
                    ...
                    '<li>산행 난이도:'+PMNTN_DFFL+'</li>'+
                    '<div style="font-size: xx-small;color: #8a8a8a">지도를 클릭하면 창이 닫힙니다.</div></div>'
                ,
        ...
        function makeClickListener(map, marker, infowindow) {
            ...
            return function() {
                ...
                for(let i=0;i<count_spot;i++) {
                    if (spot_path[i][0].PMNTN_SN === markerindex) //마커의 코스코드와 좌표의 코스코드가 같다면 저장
                    { linePath=[];
                        for(let j=0;j<spot_path[i].length;j++)
                            linePath.push(new kakao.maps.LatLng(spot_path[i][j].paths_x, spot_path[i][j].paths_y));}
                }
                console.log("linePath",linePath);
...
// 인포윈도우를 닫는 클로저를 만드는 함수입니다
    ...
    useEffect(()=>{
        ...
        /**POST**/
        axios.post(`${API_END_POINT}/sanmoa/totalroute`, {
            keyword:keyword
        })
        ...
        <div>
            ...
            <header id="search_box">
                <form onSubmit={onSubmit} >
                    <input type="text" placeholder={"산을 입력해주세요."} id="keyword" onChange={ (e)=>{ setKeyward(e.target.value)} }/>
